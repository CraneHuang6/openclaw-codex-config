#!/usr/bin/env python3
"""Runtime hard-cut for Codex/Claude model switch with monitor kept on mini.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Runtime hard-cut helper for Codex + Claude")
    ...
    parser.add_argument("--baseline-file", default=str(codex_home / "tmp/model_hardcut_baseline.json"))

    parser.add_argument("--target-model", default="gpt-5.4")
    parser.add_argument(
        "--claude-target-model",
        help="Expected Claude first-request model. Defaults to --target-model.",
    )
    ...
    if not args.claude_target_model:
        args.claude_target_model = args.target_model
    return args
...
def verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    claude_mismatch = [
        ...
        if v["first_model"] != args.claude_target_model
    ]
    ...
    result = {
        ...
        "claude_target_model": args.claude_target_model,
        "codex": {
