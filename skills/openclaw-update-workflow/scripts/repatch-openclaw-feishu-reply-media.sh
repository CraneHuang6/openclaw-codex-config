...
PATCHER="$SCRIPT_DIR/patch-openclaw-feishu-reply-media.mjs"

show_help() {
  cat <<'EOF'
Usage: repatch-openclaw-feishu-reply-media.sh [--dry-run|--apply] [--target-root <dir>]

Options:
  --dry-run              Preview patch result without writing files.
  --apply                Apply patch to reply-dispatcher.ts and create reply-dispatcher.ts.bak if needed.
  --target-root <dir>    Override target root (default: /opt/homebrew/lib/node_modules/openclaw/extensions/feishu/src).
  -h, --help             Show this help message.

Patch effects:
  - Reply text supports media fallback parsing (`Saved: ./...png`, raw local paths, Markdown `![...](...)`).
  - Extracted media paths are sent via media API and removed from text body to avoid path echo.
EOF
}
