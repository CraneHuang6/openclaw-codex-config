...
def branch_next_action(state: str) -> str:
    if state == "ready":
        return "eligible for future execute after explicit approval"
    if state == "stale":
        return "refresh branch on latest base and rerun dry-run"
    if state == "manual_review":
        return "perform manual review or cherry-pick selective commits"
    return "fix validation blockers and rerun dry-run"


def overall_next_action(candidate_branches: list[str], filtered_out: list[dict], summary: dict) -> str:
    if summary["blocked"]:
        return "fix blocked branches or exclude them, then rerun dry-run"
    if summary["manual_review"]:
        return "review manual_review branches, resolve overlap/risk, then rerun dry-run"
    if summary["stale"]:
        return "refresh stale branches on latest base and rerun dry-run"
    if not candidate_branches and filtered_out:
        return "inspect filtered_out branches; no candidate branches remain"
    if not candidate_branches:
        return "no candidate branches found for current selection"
    return "after explicit approval, use exact_next_command for legacy execution"
...
def main() -> int:
    ...
    changed_sets = {
        item["branch"]: set(item.get("changed_files", []))
    ...
    for item in inventory["branches"]:
        ...
        if classification["stale"]:
            state = "stale"
            blocker = (
                f"branch age {classification['age_days']}d exceeds stale threshold "
                f"{classification['stale_days']}d"
            )
            ...
            state = "manual_review"
            blocker = f"overlaps with {', '.join(entry['branch'] for entry in overlap)}"
        ...
        records.append(
            {
                ...
                "blocker": blocker,
                "next_action": branch_next_action(state),
    ...
    merge_order = [
        ...
        for record in sorted(
            records,
            key=lambda record: (
                category_rank(record["classification"]["category"]),
                record["behind"],
                record["branch"],
            ),
        )
    ...
    blockers = [
        {"branch": record["branch"], "state": record["state"], "reason": record["blocker"]}
        for record in records
        if record["state"] != "ready"
    ]
    validation_summary = {
        "total_candidates": len(records),
        "ready": sum(record["state"] == "ready" for record in records),
        "blocked": sum(record["state"] == "blocked" for record in records),
        "manual_review": sum(record["state"] == "manual_review" for record in records),
        "stale": sum(record["state"] == "stale" for record in records),
    }
    next_action = overall_next_action(candidate_branches, filtered_out, validation_summary)
    ...
    if candidate_branches:
        md_lines.extend([f"- `{branch}`" for branch in candidate_branches])
    else:
    ...
    if merge_order:
        md_lines.extend([f"{index}. `{branch}`" for index, branch in enumerate(merge_order, start=1)])
    else:
    ...
    md_lines.extend([
        "",
        "## Validation Summary",
        f"- total_candidates: `{validation_summary['total_candidates']}`",
        f"- ready: `{validation_summary['ready']}`",
        f"- blocked: `{validation_summary['blocked']}`",
        f"- manual_review: `{validation_summary['manual_review']}`",
        f"- stale: `{validation_summary['stale']}`",
        "",
        "## Summary Table",
        "",
    ...
    for row in records:
        overlap_summary = ", ".join(
            f"{entry['branch']}({entry['count']})" for entry in row["overlap_summary"]
        ) or "none"
    ...
    md_lines.extend(["", "## Next Action", f"- {next_action}"])
